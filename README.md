...
binary, categorical, continuous = classify_variables(
    df, ["revenue", "has_grant", "sector"], categorical_threshold=20
)

table = balance_table(
    df, binary, categorical, continuous,
    group_var="treated",                             # must be coded 0/1
...
For a table over the whole sample, use `descriptive_table(df, binary,
categorical, continuous)` instead.

Run `python demo_balance_table.py` for a complete worked example on synthetic
data; it writes both tables to `tables/`.
...
## Tests
demo_balance_table.py provides a minimal working example with a synthetic dataset.
